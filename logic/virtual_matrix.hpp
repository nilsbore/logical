...
namespace logic {
	...
	public:
		...
		enum matrix_type {DENSE, SPARSE, SCALAR, STRING} type;
		
		virtual value_type& operator() (unsigned, unsigned) = 0;
		virtual const value_type& operator() (unsigned, unsigned) const = 0;
		virtual value_type& operator[] (unsigned) = 0;
		virtual const value_type& operator[] (unsigned) const = 0;
		...
		virtual self& operator-= (const self&) = 0;
		virtual self& operator/= (const self&) = 0;
		virtual self* product(const self&) const = 0;
		/*virtual self& operator^= (self&) {} = 0;*/
		
		virtual self& operator+= (const virtual_recursator<value_type, false>&) = 0;
		virtual self& operator*= (const virtual_recursator<value_type, false>&) = 0;
		virtual self& operator-= (const virtual_recursator<value_type, false>&) = 0;
		virtual self& operator/= (const virtual_recursator<value_type, false>&) = 0;
		virtual self* product(const virtual_recursator<value_type, false>&) const = 0;
		
		virtual self& operator+= (const virtual_recursator<value_type, true>&) = 0;
		virtual self& operator*= (const virtual_recursator<value_type, true>&) = 0;
		virtual self& operator-= (const virtual_recursator<value_type, true>&) = 0;
		virtual self& operator/= (const virtual_recursator<value_type, true>&) = 0;
		virtual self* product(const virtual_recursator<value_type, true>&) const = 0;
